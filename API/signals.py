...
@receiver(post_save, sender=Glos)
def nowy_glos(sender, **kwargs):
    instance = kwargs['instance']
    channel_layer = get_channel_layer()

    vote={'Uzytkownik': instance.uzytkownik.username, 'wynik': instance.wynik, "czas": instance.czas_oddania.strftime('%H:%M:%S')}
    print("vote "+ str(vote)+ 'type'+ str(type(vote)))
